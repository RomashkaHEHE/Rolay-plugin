...
Ongoing updates are deliberately autonomous. Updater-enabled clients check immediately after
plugin load on a non-blocking deferred task, every 15 minutes after a successful check, and after
connectivity recovery. Before the first successful startup check, offline/deferred checks retry
with delays capped at one minute instead of falling through to the periodic interval. They download
and verify a newer release, wait for sync/editor activity to reach a safe idle window, persist
local state, install, and retry temporary failures without a required user action. A compact
optional `Check now` diagnostic is allowed for release testing, but it must only trigger the same
verified automatic pipeline. Do not add `Force update`, a verification bypass, or any separate
manual install path. Normal update work is silent. Only a required Obsidian restart or a persistent
automatic-retry failure should attract attention.
